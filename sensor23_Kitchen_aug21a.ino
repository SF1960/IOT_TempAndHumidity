...
/*
  Some parts of the Sketch generated by the Arduino IoT Cloud Thing "Xiao Weather"
...
#include <Wire.h>                                 // I2C communication for connecting to the DHT11:
...
#include <time.h>                                 // time functions: https://www.arduino.cc/reference/en/libraries/time/
#include "WiFi.h"                                 // library for WiFi: https://www.arduino.cc/reference/en/libraries/wifi/
#include <Preferences.h>                          // used to store boot counter in EEPROM
...
// Add these declarations at the top of your sketch, after the includes
void displayPrint(int size, int x, int y, String text);
void drawHeaderLine();
void showCopyright();
void checkUptimeRestart();

// Add any other missing function declarations here

Preferences preferences;

/*****************************************************************************
'* Program debugging
'* Set DEBUG to 0 to disable compiling of Serial.print statements
'****************************************************************************/
...
// Check if restart time has been exceeded 
// if so, restart ESP32
void checkUptimeRestart() {
  // Check if device has been running for longer than RESTART_INTERVAL
  if ((millis() - startTime) >= RESTART_INTERVAL) {
    Serial.println("Performing uptime-based restart after 24 hours...");
    
    // Clean display
    display.clearDisplay();
    displayPrint(MEDIUM, 0, 0, "RESTARTING");
    displayPrint(SMALL, 0, 20, "After 24 hours uptime");
    displayPrint(SMALL, 0, 40, "Transfers: " + String(transferNumber));
    display.display();
...
bool getTemperature() {
  ...
  if (dht.getStatus() != 0) {                     // Check if any reads failed and exit early (to try again).
    ...
    Serial.printf("%s DHT11 Error Status: %s", String(dht.getStatusString()));
...
void obtainTime() {
 ...
 // print whole time string on the monitor
 //Serial.println(&timeinfo, "%A, %B %d %Y %H:%M:%S");

  // Calculate if we're in BST (British Summer Time) or GMT
  ...
  // If we're between April and September, definitely in BST
  if (month > 3 && month < 10) {
    isBST = true;
  }
  // Handle March (month 3) - BST starts on last Sunday
  else if (month == 3) {
    // Calculate day of last Sunday (31 - day of week of 31st)
    int lastSunday = 31 - ((5 + 31 + year + (year/4) - (year/100) + (year/400)) % 7);
    
    // If we're at or past the last Sunday AND it's not before 1am UTC
    if ((timeinfo.tm_mday > lastSunday) || 
        (timeinfo.tm_mday == lastSunday && timeinfo.tm_hour >= 1)) {
      isBST = true;
    }
  }
  // Handle October (month 10) - BST ends on last Sunday
  else if (month == 10) {
    // Calculate day of last Sunday (31 - day of week of 31st)
    int lastSunday = 31 - ((2 + 31 + year + (year/4) - (year/100) + (year/400)) % 7);
    
    // If we're before last Sunday OR it's the last Sunday but before 1am UTC
    if ((timeinfo.tm_mday < lastSunday) || 
        (timeinfo.tm_mday == lastSunday && timeinfo.tm_hour < 1)) {
      isBST = true;
    }
...
void setup() {
  ...
  startTime = millis();                                  // store the value of millis()
  
  serial::setup(115200);                                 // set up the serial monitor
  sensor::setup();                                       // start DHT11
  oled::setup();                                         // start the OLED screen

  preferences.begin("sensor23", false);                  // open memory as RW

  // Get the counter value, if the key does not exist, return a default value of 0
  // The number of device BOOTING occurences
  // Note: Key name is limited to 15 chars.
  unsigned int counter = preferences.getUInt("counter", 0);
  const char * deviceName = preferences.getString ("devName", "put the device data here").c_str();
  
  // Increase boot counter by 1
  counter++;

  // Print the device name and counter to Serial Monitor
  Serial.printf(">>>> [%s] Current boot counter value: %u\n", deviceName,counter);

  // Store the counter to the Preferences
  preferences.putUInt("counter", counter);
  //preferences.putString("devName", deviceName); // only needed the first time the code is run
  ...
  */
  ...
  Serial.printf("\n");
  Serial.printf(">>>> Waiting for Sensor to settle before retrieving first data...\n");
  Serial.printf("\n");
...
void loop() {
  ...
  Serial.printf("Time:%s SSID:%s Transfer:#%i Temp:%.1fC Hum:%.0f%%RH %s\n",ntpTime,SSID,transferNumber,xiao_19_temperature,xiao_19_humidity,xiao_19_comfort);
  ...
  checkUptimeRestart();                    // check restart
...
void showCopyright() {                      // display the copyright text on last line
  displayPrint(SMALL, 31, 57, copyright);
}
...
void displayWelcome(){
  ...
  displayPrint(SMALL, 10, 45, "Device: " + device);
